...
if [ "${SMD401_OPENMPI_RUNTIME_CONFIRMED:-}" != YES ]; then
	/usr/bin/printf '%s\n' \
		'error: set SMD401_OPENMPI_RUNTIME_CONFIRMED=YES after approval' >&2
	exit 64
fi

script_dir=$(CDPATH= cd -- "$(/usr/bin/dirname "$0")" && /bin/pwd)
source_root=$(CDPATH= cd -- "${script_dir}/../.." && /bin/pwd)
source_driver=${source_root}/contribs/macos-tests/smd401_openmpi_runtime_driver.sh
...
mpi_generic=${slurm_prefix}/lib/slurm/mpi_pmix.so
...
run_dir=/private/tmp/slurm-smd401-openmpi-runtime.${run_stamp}.$$
record_dir=${run_dir}/records
client_conf=${run_dir}/slurm-client.conf
probe=${run_dir}/smd401-openmpi-probe
success_name=smd401-openmpi-success-${run_stamp}
cancel_name=smd401-openmpi-cancel-${run_stamp}
success_job=
cancel_job=
cancel_client_pid=
slurmd_pid=
...
check_success_output()
{
	...
	[ "$(/usr/bin/grep -Ec '^mpi_runtime=PASS ' "$file")" = \
		"$task_count" ] || fail "$launcher output count mismatch"
	/usr/bin/grep -Eq \
		"^mpi_runtime=PASS launcher=${launcher} job_id=${job_id} "\
"rank=0 size=2 bcast=42 allreduce=1 ring_from=1 ring_value=1001 "\
"processor=[^ ]+ ompi=5.0.11$" \
		"$file" || fail "$launcher rank 0 result mismatch"
	/usr/bin/grep -Eq \
		"^mpi_runtime=PASS launcher=${launcher} job_id=${job_id} "\
"rank=1 size=2 bcast=42 allreduce=1 ring_from=0 ring_value=1000 "\
"processor=[^ ]+ ompi=5.0.11$" \
		"$file" || fail "$launcher rank 1 result mismatch"
}
...
related_processes()
{
	/bin/ps -axo pid=,ppid=,pgid=,state=,comm= |
		/usr/bin/awk '
			$5 ~ /(^|\/)(prte|mpirun|smd401-openmpi-probe)$/ {
				print
			}
		'
}
...
for required in "$source_driver" "$probe_source" "$mpicc" "$mpirun" \
	"$libmpi" "${pmix_prefix}/bin/pmix_info" "$slurm_conf" "$scontrol" \
	"$squeue" "$srun" "$scancel" "$sacct" "$mpi_plugin" \
	"$mpi_generic" "$pid_file" "$slurmd_log"; do
...
"$mpicc" --showme >"${run_dir}/mpicc-showme.txt"
...
if [ "$earlier" = "$slurmd_start" ] && \
	[ "$slurmd_start" != "$plugin_mtime" ]; then
...
"$srun" --mpi=list >"${run_dir}/mpi-list.txt" \
	2>"${run_dir}/mpi-list.err" || fail 'cannot list MPI plugins'
...
/usr/bin/shasum -a 256 "$slurm_conf" "$source_driver" "$probe_source" \
	"$mpi_plugin" "$mpi_generic" "$srun" "$mpicc" "$mpirun" "$libmpi" \
...
/usr/bin/sudo -n -H -u "$test_user" /usr/bin/env \
	PATH=/opt/homebrew/bin:/usr/bin:/bin TMPDIR=/private/tmp \
	"$mpirun" --bind-to none -n "$task_count" "$probe" success \
	>"${run_dir}/local.out" 2>"${run_dir}/local.err"
local_rc=$?
[ "$local_rc" -eq 0 ] || fail "local Open MPI control failed rc=$local_rc"
[ ! -s "${run_dir}/local.err" ] || fail 'local Open MPI stderr is not empty'
check_success_output "${run_dir}/local.out" local none
related_processes >"${run_dir}/mpi-processes-after-local.txt"
[ ! -s "${run_dir}/mpi-processes-after-local.txt" ] || \
	fail 'MPI-related process remains after local control'
/usr/bin/printf 'openmpi_local=PASS tasks=%s bcast=42 allreduce=1 ring=PASS\n' \
	"$task_count"

/usr/bin/sudo -n -H -u "$test_user" /usr/bin/env \
	PATH=/opt/homebrew/bin:/usr/bin:/bin SLURM_CONF="$client_conf" \
	"$srun" --partition="$partition" --nodes=1 --ntasks="$task_count" \
	--cpus-per-task=1 --mem=256M --time=00:01:00 \
	--chdir=/private/tmp --mpi=pmix_v6 --kill-on-bad-exit=1 \
	--job-name="$success_name" "$probe" success \
	>"${run_dir}/success.out" 2>"${run_dir}/success.err"
success_rc=$?
[ "$success_rc" -eq 0 ] || fail "Open MPI success run failed rc=$success_rc"
[ ! -s "${run_dir}/success.err" ] || \
	fail 'Open MPI success stderr is not empty'
...
check_success_output "${run_dir}/success.out" slurm "$success_job"
...
/usr/bin/printf \
	'openmpi_slurm_success=PASS job_id=%s tasks=%s communication=PASS\n' \
	"$success_job" "$task_count"

/usr/bin/sudo -n -H -u "$test_user" /usr/bin/env \
	PATH=/opt/homebrew/bin:/usr/bin:/bin SLURM_CONF="$client_conf" \
	"$srun" --partition="$partition" --nodes=1 --ntasks="$task_count" \
	--cpus-per-task=1 --mem=256M --time=00:02:00 \
	--chdir=/private/tmp --mpi=pmix_v6 --kill-on-bad-exit=1 \
	--job-name="$cancel_name" "$probe" hold "$record_dir" \
...
[ "$(queue_state "$cancel_job")" = RUNNING ] || \
	fail 'cancel job is not RUNNING'
...
"$scancel" "$cancel_job" || fail 'cannot cancel Open MPI allocation'
...
/usr/bin/printf \
	'openmpi_slurm_cancel=PASS job_id=%s tasks=%s client_rc=%s cleanup=PASS\n' \
...
/usr/bin/printf '%s%s%s%s%s\n' \
	'SMD401_OPENMPI_RUNTIME_COMPLETE' \
	" local_tasks=$task_count success_job=$success_job" \
	" cancel_job=$cancel_job tasks=$task_count communication=PASS" \
	" cancel_cleanup=PASS slurmd_pid=$slurmd_pid" \
