...
if [ "${SMD122_EPILOG_FAILURE_CONFIRMED:-}" != YES ]; then
	/usr/bin/printf 'error: set SMD122_EPILOG_FAILURE_CONFIRMED=YES after confirming intentional node DRAIN is acceptable\n' >&2
	exit 64
fi
...
wait_epilog_failure()
{
	...
	while [ "$attempt" -lt 180 ]; do
		...
		*) node_drained=0 ;;
		...
		if [ "$node_drained" -eq 1 ] && [ -z "$(queue_state "$job_id")" ] && \
			accounting_complete "$job_id" "${run_dir}/failure-sacct.txt"; then
			/usr/bin/printf 'failure_observed mode=%s job_id=%s node_state=%s accounting=COMPLETED:0:0 wait_seconds=%s\n' \
				"$mode" "$job_id" "$node_state" "$attempt"
			return 0
		fi
		/bin/sleep 1
		attempt=$((attempt + 1))
...
wait_accounting_complete()
{
	...
	while [ "$attempt" -lt 60 ]; do
		if accounting_complete "$job_id" "$output_file"; then
			return 0
		fi
		/bin/sleep 1
		attempt=$((attempt + 1))
...
cleanup()
{
	...
	if [ "$success" -ne 1 ]; then
		if [ "$failure_injected" -eq 1 ]; then
			...
			/usr/bin/printf 'recovery: stopped before intentional Epilog failure injection; no failure job was submitted; inspect run_dir=%s\n' \
				"$run_dir" >&2
		fi
	fi
	exit "$rc"
...
if [ "$mode" = nonzero ]; then
	hook_source=${source_root}/contribs/macos-tests/smd122_epilog_nonzero.sh
else
	hook_source=${source_root}/contribs/macos-tests/smd122_epilog_timeout.sh
fi

for required_file in "$slurm_conf" "$slurmd" "$scontrol" "$squeue" \
	"$sbatch" "$scancel" "$sacct" "$pid_file" "$production_log" \
	"$hook_source" "${source_root}/contribs/macos-tests/smd122_payload.sh"; do
...
/bin/cp "$hook_source" "${hook_dir}/epilog.sh" || fail 'cannot stage epilog'
/bin/cp "${source_root}/contribs/macos-tests/smd122_payload.sh" "$payload" || \
	fail 'cannot stage payload'
/bin/chmod 0555 "${hook_dir}/epilog.sh" "$payload" || fail 'cannot set executable modes'
/usr/sbin/chown 0:0 "${hook_dir}/epilog.sh" "$payload" || fail 'cannot set staged owners'
...
{
	/usr/bin/printf '\n# SMD-122 temporary intentional Epilog failure test\n'
	/usr/bin/printf 'Epilog=%s\n' "${hook_dir}/epilog.sh"
	if [ "$mode" = timeout ]; then
		/usr/bin/printf 'EpilogTimeout=%s\n' "$timeout_seconds"
	fi
} >>"$candidate_conf" || fail 'cannot append candidate Epilog settings'
...
test_result=$(submit_payload "smd122-${mode}" "$payload_marker" \
...
wait_epilog_failure "$test_job" || fail 'expected Epilog DRAIN and completed accounting were not observed'
...
[ -f "$payload_marker" ] || fail 'payload marker is missing before Epilog failure'
/usr/bin/grep -Fq "SMD122_PAYLOAD_EXECUTED=YES job_id=${test_job} uid=${test_uid} gid=${test_gid}" \
	"$payload_marker" || fail 'payload marker identity mismatch'

if [ "$mode" = nonzero ]; then
	/usr/bin/grep -Eq "^event=epilog_nonzero job_id=${test_job} epoch=[0-9]+ euid=0 egid=0 context=epilog_slurmd exit_code=42$" \
		"$event_log" || fail 'nonzero Epilog event mismatch'
else
	/usr/bin/grep -Eq "^event=epilog_timeout_begin job_id=${test_job} epoch=[0-9]+ euid=0 egid=0 context=epilog_slurmd pid=[0-9]+ pgid=[0-9]+$" \
		"$event_log" || fail 'timeout Epilog event mismatch'
	timeout_pid=$(/usr/bin/awk -v job="$test_job" '
		$1 == "event=epilog_timeout_begin" && $2 == "job_id=" job {
			for (i = 1; i <= NF; i++) if ($i ~ /^pid=/) {
				sub(/^pid=/, "", $i); print $i; exit
			}
		}' "$event_log")
	timeout_pgid=$(/usr/bin/awk -v job="$test_job" '
		$1 == "event=epilog_timeout_begin" && $2 == "job_id=" job {
			for (i = 1; i <= NF; i++) if ($i ~ /^pgid=/) {
				sub(/^pgid=/, "", $i); print $i; exit
			}
		}' "$event_log")
...
/usr/bin/sed -n "${log_first},\$p" "$production_log" >"${run_dir}/slurmd-failure.log"
if [ "$mode" = nonzero ]; then
	/usr/bin/grep -Eq 'epilog failed status=42:0' "${run_dir}/slurmd-failure.log" || \
		fail 'worker log lacks Epilog exit 42 evidence'
else
	/usr/bin/grep -Eq 'epilog poll timeout @ 5000 msec|timeout after 5000 ms|epilog.*timed out' \
		"${run_dir}/slurmd-failure.log" || fail 'worker log lacks Epilog timeout evidence'
	/usr/bin/grep -Eq 'epilog failed status=0:15|epilog killed by signal 15' \
		"${run_dir}/slurmd-failure.log" || fail 'worker log lacks Epilog signal evidence'
fi
/usr/bin/printf 'failure_state=PASS mode=%s job_id=%s node_reason=%s accounting=COMPLETED:0:0 payload_executed=YES\n' \
	"$mode" "$test_job" "$node_reason"
...
smoke_result=$(submit_payload "smd122-${mode}-smoke" "$smoke_marker" \
...
/usr/bin/grep -Fq "SMD122_PAYLOAD_EXECUTED=YES job_id=${smoke_job} uid=${test_uid} gid=${test_gid}" \
...
success=1
trap - EXIT HUP INT TERM
/usr/bin/printf 'SMD122_PHASE_COMPLETE mode=%s failure_job=%s smoke_job=%s slurmd_pid=%s run_dir=%s\n' \
