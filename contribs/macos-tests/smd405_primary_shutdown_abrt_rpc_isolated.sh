...
while [ "$i" -lt 20 ] && [ ! -S "$sackd_socket" ]; do
	...
	kill -0 "$sackd_launcher_pid" 2>/dev/null || \
		fail 'isolated sackd exited before socket creation'
	sleep 1
done
[ -S "$sackd_socket" ] || fail 'isolated sackd socket was not created'

sackd_pids=$(find_isolated_sackd_pids)
sackd_count=$(printf '%s\n' "$sackd_pids" | sed '/^$/d' | wc -l | tr -d ' ')
[ "$sackd_count" -eq 1 ] || fail 'isolated sackd PID count is not one'
sackd_pid=$(printf '%s\n' "$sackd_pids" | sed -n '1p')
...
set -e
gdb_launcher_pid=
stop_isolated_sackd || fail 'isolated sackd cleanup failed'
...
if grep -Fq 'Performing RPC: REQUEST_SHUTDOWN' "${run_dir}/daemon-log/slurmctld.log"; then
	rpc_observed=YES
fi
