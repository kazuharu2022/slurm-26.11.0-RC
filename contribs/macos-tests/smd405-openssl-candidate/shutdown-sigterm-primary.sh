...
# Candidate-only SMD-405 direct SIGTERM regression. This is a new bounded
# harness; retired historical drivers remain disabled.

set -eu

if [ "${SMD405_OPENSSL_CANDIDATE_SIGTERM_CONFIRMED:-}" != YES ]; then
	printf '%s\n' \
		'error: set SMD405_OPENSSL_CANDIDATE_SIGTERM_CONFIRMED=YES after reviewing the isolated scope' >&2
	exit 64
fi
...
isolated_port=16927
isolated_slurmd_port=16928
active_marker=/var/tmp/smd405-openssl-candidate-sigterm-active
run_stamp=$(date '+%Y%m%dT%H%M%S')
run_dir=/var/tmp/smd405-openssl-candidate-sigterm-${run_stamp}
...
fail()
{
	printf 'SMD405_OPENSSL_CANDIDATE_SIGTERM_FAILED error=%s run_dir=%s\n' \
		"$1" "$run_dir" >&2
	exit 1
}
...
# SMD-405 OpenSSL candidate isolated SIGTERM configuration
ClusterName=smd405-openssl-candidate-sigterm
...

{
	...
	production_command "$production_scontrol" ping
	ss -H -ltn | awk '$4 ~ /:(6817|16927)$/ { print }'
...
[ -r "/proc/${isolated_pid}/cmdline" ] || fail 'isolated PID disappeared before SIGTERM'
tr '\000' ' ' <"/proc/${isolated_pid}/cmdline" | grep -Fq "$isolated_conf" || \
	fail 'SECURITY: SIGTERM target no longer matches isolated config'
[ "$isolated_pid" != "$production_ctld_pid" ] || \
	fail 'SECURITY: SIGTERM target equals production PID'
ss -H -ltn | awk -v port=":${isolated_port}" \
	'$4 ~ port "$" { found = 1 } END { exit !found }' || \
	fail 'isolated port is not listening immediately before SIGTERM'
...
finished=1
trap - EXIT HUP INT TERM
printf 'SMD405_OPENSSL_CANDIDATE_SIGTERM_RESULT result=%s helper_runtime=%s production=UNCHANGED sack=UNCHANGED queue=EMPTY nodes=IDLE jobs=NONE run_dir=%s\n' \
	"$result" "$helper_runtime" "$run_dir"
[ "$result" = NOT_REPRODUCED_CLEAN_SIGTERM ] || {
	printf 'SMD405_OPENSSL_CANDIDATE_SIGTERM_FAILED result=%s run_dir=%s\n' \
		"$result" "$run_dir" >&2
	exit 1
}
printf '%s\n' 'SMD405_OPENSSL_CANDIDATE_SIGTERM_COMPLETE'
