...
for value in "$job_id" "$step_id" "$proc_id" "$local_id" "$node_id" \
	"$task_count" "$step_task_count" "$node_count" "$pid"; do
	case "$value" in
	''|*[!0-9]*) exit 92 ;;
	esac
done
...
/usr/bin/printf '%s%s%s%s%s%s\n' \
	"record=cpu-task case=${case_label} job_id=${job_id}" \
	" step_id=${step_id} procid=${proc_id} localid=${local_id}" \
	" nodeid=${node_id} ntasks=${task_count}" \
	" step_ntasks=${step_task_count} nnodes=${node_count} pid=${pid}" \
